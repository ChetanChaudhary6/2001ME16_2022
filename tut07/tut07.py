...
def octant_range_names(mod, filename):
    ...
    # Reading Excel File
    try:
        df = pd.read_excel(f'C:\\Users\\user\\OneDrive\\Documents\\Downloads\\tut07\\input\\{filename}')
        rows = df.shape[0]
...
def octant_longest_subsequence_count_with_range(mod, df, filename):
    ...
    try:
        # Exporting dataframe to excel 
        df.to_excel(f'C:\\Users\\user\\OneDrive\\Documents\\Downloads\\tut07\output\\{filename[0:len(filename)-5]}_vel_octant_analysis_mod_{mod}.xlsx', index=False)
        return df
    except Exception as e:
...
def octant_analysis(mod):
    dir_list = os.listdir('C:\\Users\\user\\OneDrive\\Documents\\Downloads\\tut07\\input') # Making list of input files
    for filename in dir_list:
        ...
        path='C:\\Users\\user\\OneDrive\\Documents\\Downloads\\tut07\\input\\'+filename  
        outPath='C:\\Users\\user\\OneDrive\\Documents\\Downloads\\tut07\\output\\'+str(filename[0:len(filename)-5])+'_vel_octant_analysis_mod_'+str(mod)+'.xlsx'        
        ...
        for row in sheet.iter_rows(min_row=2, min_col=1, max_row=tot_r+1, max_col=tot_c):
            ...
            for cell in row: 
                ...
                c+=1
            r+=1
        for row in sheet.iter_rows(min_row=1, min_col=14, max_row=total_rows+2, max_col=32):
            for cell in row: 
        ...
        for i in range (total_rows+7,total_rows+16):
        ...
        for ro in range(2, tot_r):
            # print(str(sheet.cell(row = ro, column = 50).value))
